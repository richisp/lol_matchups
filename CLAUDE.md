...
| [scrape_wiki_ratings.py](scrape_wiki_ratings.py) | Playwright scraper for the LoL wiki's `List_of_champions/Ratings` page (bot-challenged — plain HTTP and even its api.php are blocked, so a real browser is required; crawler-side only). Extracts **priority-ordered** classes (Primary/Secondary columns) + the five 0-3 ratings; stores with `roles_ranked=1`. Self-skips unless `champion_stats` has champions without wiki-ranked attribute rows (wiki class data changes very rarely) — `--force` for reworks. Refuses to write below 150 usable rows; alternate-form rows (Mega Gnar, Kled & Skaarl) are skipped by ddragon name-matching. |
...
champion_attributes (champion_name, riot_id, roles, damage, toughness, control,
   ...
   roles_ranked: 1 = roles are priority-ordered (wiki Primary/Secondary; also set
   by user overrides); 0 = alphabetical (Meraki/CDragon). Unordered upserts never
   overwrite ranked roles (CASE guard in db.upsert_champion_attributes).
...
- `compute_blind_risk()`: popularity-weighted exposure to bad **counter** matchups (winrate < `BLIND_PICK_BAD_WR_THRESHOLD = 48.0`) across enemy lanes — `Σ over enemy lanes of SUM(pickrate of bad matchups) × COUNTER_WEIGHTS[lane][L]/100`. Synergy is deliberately excluded (a poorly-fitting ally isn't something the enemy can exploit; a synergy term was tried and removed 2026-07 along with the old `/2` rescale). Lanes already filled by the enemy are excluded via `known_enemy_lanes` — their matchups are known and live in the `vs` term — so risk fades to 0 as the enemy team locks in. **Risk feeds the fit score**: `fit −= RISK_WEIGHT (0.1) × blind_risk`, so early-draft rankings favor safe blind picks and converge to pure matchup scoring by the end of draft; the deduction shows in the breakdown tooltip next to the risk value. UI color bands (in `draft.html`): green `<10`, red `>40` (counter-only distribution: per-lane medians 11–29, close to the old halved counter+synergy scale). Lower = safer blind pick. The general champion list passes no team context, so it reflects raw all-lanes exposure.
...
Champions get a soft 0–1 fit per comp archetype (`engage`, `poke`, `protect` — three macro comps; dive/pick/wombo are engage variations, f2b/peel/disengage are the protect family) from `compute_comp_fits()`: base = **max** over the champ's Riot **subclass** tags in `config.SUBCLASS_COMP_FIT` with hybrid damping, plus small attribute nudges (mobility/control → engage, utility → protect), clamped to [0,1]. Damping has two modes keyed off `champion_attributes.roles_ranked`: **1** (wiki-scraped, priority-ordered Primary/Secondary) → per-position multipliers `SUBCLASS_RANK_DAMPING` (1: ×1.0; 2: primary ×0.7, secondary ×0.5); **0** (Meraki/CDragon, alphabetical) → uniform count damping `SUBCLASS_COUNT_DAMPING` (1 ×1.0, 2 ×0.7, 3+ ×0.5). Base classes (FIGHTER/MAGE/TANK/SUPPORT) are dropped everywhere — subclasses are strictly more accurate. Subclasses carry what ratings can't: CC *direction* (Vanguard engages, Warden peels).
...
- Comp fits feed the fit score **only when a target comp is manually selected**: candidates get `+10 × fit(selected comp)` added to Score (a "Comp bonus" row appears in their tooltips). Auto mode is display-only, and picked-slot scores / team averages stay pure matchup math in both modes.
